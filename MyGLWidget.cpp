...
void MyGLWidget::pintaMuntanyes(){
    ...
    transformacioMuntanya(0.5, this->xSegonaMuntanya, 1);
    glDrawArrays(GL_TRIANGLE_FAN, 0, MUNTANYA_NUM_VERTEXS);
...
void MyGLWidget::keyPressEvent(QKeyEvent* event)
{
    ...
    switch (event->key()) {
        case Qt::Key_A: {
            if (-0.5 < this->anglePalanca) {
              this->anglePalanca -= 0.1;
            }
            break;
        }
        case Qt::Key_D: {
            if (this->anglePalanca < 0.4) {
              this->anglePalanca += 0.1;
            }
            break;
        }
...
void MyGLWidget::creaBufferMuntanya(){
    ...
    glm::vec3 Vertices[MUNTANYA_NUM_VERTEXS]= {
     glm::vec3(0.0, 1.0, 0.0), // pico
     glm::vec3(-1.0, -1.0, 0.0), // abajo izquierda
     glm::vec3(1.0, -1.0, 0.0), // abajo derecha
     glm::vec3(h2/2, 1-h2, 0.0), // creo que empieza la nieve
...
void MyGLWidget::carregaShaders()
{
  ...
  transLoc = glGetUniformLocation(program->programId(), "transLoc");
  transparenciaLoc = glGetUniformLocation(program->programId(), "transparenciaLoc");
